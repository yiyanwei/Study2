...
ORDER BY
	t1.relname ASC";
        ...
        /// <returns></returns>
        string GetTypeSimpleNameByString(string type)
        {
            switch (type.ToLower())
            {
                case "system.boolean":
...
WHERE
	...
	AND A.attnum >0", null);
            ...
            StringBuilder codeString)
        {
            ...
            codeString.Append($@"{(string.IsNullOrWhiteSpace(settings.Namespace) ? "" : $"{Environment.NewLine}{settings.Namespace.Trim()} using SqlSugar;")}
...
");
                    ...
                    {
                        codeString.Replace("-dbType-", isYuLan ? $"<span style=\"color:#23C645\">{GetTypeSimpleNameByString(dr[dataTypeName].ToString())}?</span>" : GetTypeSimpleNameByString(dr[dataTypeName].ToString()) + "?");  //替换数据类型
                        if (settings.PropDefault)
                        {
                            codeString.Replace("-value-", $"value ?? default({(isYuLan ? $"<span style=\"color:#23C645\">{GetTypeSimpleNameByString(dr[dataTypeName].ToString())}</span>" : GetTypeSimpleNameByString(dr[dataTypeName].ToString()))})");
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            codeString.Replace("-dbType-", isYuLan ? $"<span style=\"color:red\">{GetTypeSimpleNameByString(dr[dataTypeName].ToString())}</span>" : GetTypeSimpleNameByString(dr[dataTypeName].ToString()));  //替换数据类型
